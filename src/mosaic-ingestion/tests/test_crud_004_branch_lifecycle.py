...
from utils.commit_state_manager import CommitState
...
class TestBranchLifecycleManagerInitialization:
    ...
    def test_branch_lifecycle_manager_initialization(
        self, mock_commit_state_manager, mock_cosmos_client
    ):
        ...
        manager = BranchLifecycleManager(commit_state_manager=mock_commit_state_manager)
...
class TestBranchCreation:
    ...
    async def test_create_branch_duplicate_name(
        self, temp_git_repo, mock_branch_manager
    ):
        ...
        with pytest.raises(ValueError, match="already exists"):
            await mock_branch_manager.create_branch(
                repo=repo, repository_url=repository_url, branch_name=branch_name
            )

    @pytest.mark.asyncio
    async def test_create_branch_with_parent_commit(
        self, temp_git_repo, mock_branch_manager
    ):
...
class TestBranchSwitching:
    ...
    @pytest.mark.asyncio
    async def test_switch_to_existing_branch(
        self, temp_git_repo_with_branches, mock_branch_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_switch_to_nonexistent_branch_create_if_missing(
        self, temp_git_repo_with_branches, mock_branch_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_switch_to_nonexistent_branch_no_create(
        self, temp_git_repo_with_branches, mock_branch_manager
    ):
...
class TestBranchDeletion:
    ...
    @pytest.mark.asyncio
    async def test_delete_active_branch_raises_error(
        self, temp_git_repo_with_branches, mock_branch_manager
    ):
        ...
        with pytest.raises(ValueError, match="Cannot delete currently active branch"):
            await mock_branch_manager.delete_branch(
                repo=repo, repository_url=repository_url, branch_name=active_branch
            )

    @pytest.mark.asyncio
    async def test_delete_nonexistent_branch_raises_error(
        self, temp_git_repo_with_branches, mock_branch_manager
    ):
...
class TestConflictDetection:
    ...
    async def test_detect_conflicts_found(
        self, temp_git_repo_with_conflicts, mock_branch_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_detect_conflicts_none_found(
        self, temp_git_repo_with_branches, mock_branch_manager
    ):
    ...
    @pytest.mark.asyncio
    async def test_detect_conflicts_nonexistent_branch(
        self, temp_git_repo_with_branches, mock_branch_manager
    ):
...
class TestBranchListing:
    ...
    @pytest.mark.asyncio
    async def test_list_branches_without_metadata(
        self, temp_git_repo_with_branches, mock_branch_manager
    ):
        ...
        branches = await mock_branch_manager.list_branches(
            repo=repo, repository_url=repository_url, include_metadata=False
        )
    ...
    @pytest.mark.asyncio
    async def test_list_branches_with_metadata(
        self, temp_git_repo_with_branches, mock_branch_manager
    ):
        ...
        branches = await mock_branch_manager.list_branches(
            repo=repo, repository_url=repository_url, include_metadata=True
        )
...
class TestIngestionIntegration:
    ...
    @pytest.mark.asyncio
    async def test_setup_branch_for_processing_existing(
        self, temp_git_repo_with_branches, mock_branch_manager
    ):
        ...
        mock_branch_manager.commit_state_manager.get_last_commit = AsyncMock(
            return_value=mock_commit_state
        )
        ...
        (
            branch_metadata,
            commit_state,
        ) = await mock_branch_manager.setup_branch_for_processing(
            repo=repo,
            repository_url=repository_url,
            branch_name=branch_name,
            create_if_missing=True,
    ...
    @pytest.mark.asyncio
    async def test_setup_branch_for_processing_create_missing(
        self, temp_git_repo_with_branches, mock_branch_manager
    ):
    ...
    async def test_get_branch_for_ingestion(self, mock_branch_manager):
        ...
        mock_branch_manager.get_branch_metadata = AsyncMock(
            return_value=mock_branch_metadata
        )
        mock_branch_manager.commit_state_manager.get_last_commit = AsyncMock(
            return_value=mock_commit_state
        )
        ...
        (
            branch_metadata,
            commit_state,
        ) = await mock_branch_manager.get_branch_for_ingestion(
            repository_url=repository_url, branch_name=branch_name
        )
...
class TestBranchMetadataPersistence:
    ...
    async def test_get_branch_metadata_found(self, mock_branch_manager):
        ...
        # Get metadata
        metadata = await mock_branch_manager.get_branch_metadata(
...
class TestErrorHandling:
    ...
    async def test_cosmos_db_connection_error(self, mock_branch_manager):
        ...
        mock_branch_manager._get_branch_item = AsyncMock(
    ...
    @pytest.mark.asyncio
    async def test_git_operation_error(
        self, temp_git_repo_with_branches, mock_branch_manager
    ):
        ...
        with pytest.raises(Exception):
            await mock_branch_manager.create_branch(
                repo=repo, repository_url="test_url", branch_name="test_branch"
            )
    ...
    async def test_status_healthy(self, mock_branch_manager):
        ...
        mock_branch_manager.branches_container.read = MagicMock(
            return_value={"id": "test_container"}
        )
        mock_branch_manager.commit_state_manager.get_status = AsyncMock(
    ...
    async def test_status_unhealthy(self, mock_branch_manager):
        ...
        mock_branch_manager.database.read = MagicMock(
            side_effect=Exception("Connection failed")
        )
