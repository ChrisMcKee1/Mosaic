...
def validate_powershell_wrapper():
    ...
    if not ps_path:
        print("❌ No PowerShell script found")
        return False
    ...
    try:
        ...
        core_elements = ["param(", "-Confirm", "-Force", "-Help"]
        ...
        if core_missing == 0:
            print(
                f"✅ PowerShell wrapper validation passed! ({optional_missing} optional features missing)"
            )
            return True
        else:
            print(
                f"❌ PowerShell wrapper validation failed! ({core_missing} core features missing)"
            )
            return False
...
def validate_test_structure():
    ...
    try:
        ...
        required_test_classes = ["TestCosmosDBCleaner", "TestCosmosCleanupIntegration"]
