...
class CosmosDBCleaner:
    ...
    def initialize_connection(self) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ Connected to Cosmos DB: {self.settings.azure_cosmos_endpoint}")
            ...
            for container_name in self.containers:
                try:
                    ...
                    if documents:
                        ...
                        logger.info(f"   ✅ Backed up {len(documents)} documents from {container_name}")
                    else:
                        backup_summary["containers"][container_name] = {
                            "document_count": 0,
                            "backup_file": None
    ...
    def cleanup_container(self, container_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for doc in documents:
                try:
                    ...
                    if deleted_count % 100 == 0:
                        logger.info(f"   🗑️ Deleted {deleted_count}/{len(documents)} documents")
            ...
            if errors:
                logger.warning(f"⚠️ {len(errors)} errors during cleanup of {container_name}")
                for error in errors[:5]:  # Log first 5 errors
                    logger.warning(f"   {error}")
            else:
                logger.info(f"✅ Successfully cleaned {container_name}: {deleted_count} documents deleted")
            ...
            for container_name in self.containers:
                ...
                if stats["exists"]:
                    if stats["document_count"] == 0:
                        ...
                        logger.warning(f"   ⚠️ {container_name}: Still contains {stats['document_count']} documents")
                        all_valid = False
                else:
        ...
        try:
            ...
            if create_backup and total_docs > 0:
                ...
                if not self.create_backup():
                    if not force:
                        response = input("Backup failed. Continue without backup? (yes/no): ")
                        if response.lower() != 'yes':
                            logger.info("❌ Cleanup cancelled due to backup failure")
                            return False
                    else:
                        logger.warning("⚠️ Backup failed but continuing due to --force flag")
            ...
            for container_name in self.containers:
                ...
                self.cleanup_stats["documents_deleted"] += result.get("documents_deleted", 0)
            ...
            logger.info(f"Containers processed: {self.cleanup_stats['containers_processed']}")
...
def main():
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Skip all confirmation prompts (for CI/CD)"
    )
