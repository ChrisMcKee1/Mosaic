...
class BranchLifecycleManager:
    ...
    def from_settings(cls, settings) -> "BranchLifecycleManager":
        ...
        return cls(commit_state_manager=commit_state_manager)
    ...
    async def create_branch(
        ...
        try:
            ...
            branch_metadata = BranchMetadata(
                ...
                author_name=repo.config_reader().get_value(
                    "user", "name", fallback="unknown"
                ),
                author_email=repo.config_reader().get_value(
                    "user", "email", fallback="unknown"
                ),
    ...
    async def switch_branch(
        ...
        try:
            ...
            if repo.is_dirty():
                logger.warning(
                    "Repository has uncommitted changes, switching may cause issues"
                )
            ...
            if current_branch != target_branch:
                await self._update_branch_active_status(
                    repository_url, current_branch, False
                )

            await self._update_branch_active_status(repository_url, target_branch, True)

            # Get updated metadata
            branch_metadata = await self.get_branch_metadata(
    ...
    async def delete_branch(
        ...
        try:
            ...
            if repo.active_branch.name == branch_name:
                raise ValueError(
                    f"Cannot delete currently active branch '{branch_name}'"
                )
            ...
            if not force:
                ...
                logger.warning(
                    f"Merge check for '{branch_name}' - using force deletion for now"
                )
    ...
    async def detect_conflicts(
        ...
        try:
            ...
            logger.info(
                f"No conflicts detected between '{source_branch}' and '{target_branch}'"
            )
            return None
    ...
    async def list_branches(
        self, repo: git.Repo, repository_url: str, include_metadata: bool = True
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            branches = []
            current_branch = (
                repo.active_branch.name if hasattr(repo.active_branch, "name") else None
            )
    ...
    async def get_branch_metadata(
        self, repository_url: str, branch_name: str
        ...
        try:
            logger.debug(
                f"Retrieving metadata for branch {repository_url}#{branch_name}"
            )
            ...
            if not item:
                logger.debug(
                    f"No metadata found for branch {repository_url}#{branch_name}"
                )
                return None
    ...
    async def _persist_branch_metadata(self, metadata: BranchMetadata) -> None:
        ...
        branch_id = self._generate_branch_id(
            metadata.repository_url, metadata.branch_name
        )
    ...
    async def _update_branch_active_status(
        self, repository_url: str, branch_name: str, is_active: bool
    ) -> None:
    ...
    async def _create_metadata_for_existing_branch(
        self, repo: git.Repo, repository_url: str, branch_name: str
    ) -> BranchMetadata:
    ...
    async def _archive_branch_metadata(
        self, repository_url: str, branch_name: str
    ) -> None:
    ...
    async def _delete_branch_metadata(
        self, repository_url: str, branch_name: str
    ) -> None:
    ...
    async def _get_branch_item(self, branch_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = self.branches_container.read_item(
                item=branch_id, partition_key=branch_id
            )
            return response
        except CosmosResourceNotFoundError:
            return None
    ...
    async def _delete_branch_item(self, branch_id: str) -> None:
        ...
        self.branches_container.delete_item(item=branch_id, partition_key=branch_id)
        ...
        try:
            ...
            if branch_name not in [head.name for head in repo.heads]:
                ...
                branch_metadata = await self.switch_branch(
                    repo=repo, repository_url=repository_url, target_branch=branch_name
                )
