...
class KnowledgeRepository(BranchAwareRepository):
    ...
    async def upsert_golden_node(
        self, golden_node: GoldenNode, ttl_override: Optional[int] = None
    ) -> GoldenNode:
        """
    ...
    async def get_golden_node(
        self, node_id: str, repository_url: str, branch_name: str
    ) -> Optional[GoldenNode]:
        """
    ...
    async def query_entities_by_file(
        self, repository_url: str, branch_name: str, file_path: str
    ) -> List[GoldenNode]:
        """
        ...
        try:
            ...
            if file_paths:
                file_placeholders = ", ".join(
                    [f"@file_{i}" for i in range(len(file_paths))]
                )
                additional_filter += (
                    f" AND c.file_context.file_path IN ({file_placeholders})"
                )
            ...
            source_items = await self.query_branch_items(
                repository_url=repository_url, branch_name=source_branch
            )

            # Build conflict detection logic
            conflicts = []
            source_by_name = {
                item["code_entity"]["name"]: item for item in source_items
            }
            ...
            for target_item in target_items:
                ...
                if source_item:
                    ...
                    if (
                        source_item["code_entity"]["content"]
                        != target_item["code_entity"]["content"]
                    ):
                        conflicts.append(
                            {
    ...
    async def get_entity_relationships(
        self, repository_url: str, branch_name: str, entity_id: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for item in items:
                for relationship in item.get("relationships", []):
                    if (
                        relationship.get("source_entity_id") == entity_id
                        or relationship.get("target_entity_id") == entity_id
                    ):
                        relationships.append(
                            {**relationship, "parent_entity_id": item["id"]}
                        )
...
class RepositoryStateRepository(BranchAwareRepository):
    ...
    async def upsert_commit_state(
        ...
        try:
            ...
            state_id = hashlib.sha256(
                f"{repository_url}#{branch_name}".encode()
            ).hexdigest()
    ...
    async def list_repository_branches(
        ...
        """
        try:
            items = await self.query_cross_branch_items(repository_url=repository_url)
...
class MemoryRepository(BranchAwareRepository):
    ...
    async def retrieve_branch_memories(
        ...
        """
        try:
            additional_filter = "c.importance_score >= @min_importance"
...
class RepositoryFactory:
    ...
    def create_knowledge_repository(
        self, container_name: str = "knowledge"
    ) -> KnowledgeRepository:
    ...
    def create_repository_state_repository(
        self, container_name: str = "repositories"
    ...
    def create_memory_repository(
        self, container_name: str = "memory"
    ) -> MemoryRepository:
