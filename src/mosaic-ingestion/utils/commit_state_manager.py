...
class CommitStateManager:
    ...
    async def get_last_commit(
        ...
        try:
            ...
            if not item:
                logger.info(
                    f"No previous commit state found for {repository_url}#{branch_name}"
                )
                return None
    ...
    def iter_commits_since_last(
        ...
        try:
            ...
            if not last_commit_sha:
                ...
                logger.info(
                    f"First-time processing: found {len(commits_to_process)} total commits"
                )
            else:
    ...
    def get_commit_diff(
        ...
        try:
            ...
            logger.debug(
                f"Found {len(diff_list)} changed files in commit {commit.hexsha[:8]}"
            )
