    ...
    def get_indexing_policy() -> Dict[str, Any]:
        ...
        return {
            ...
            "compositeIndexes": [
                ...
                [
                    {"path": "/processing_metadata/processing_stage", "order": "ascending"},
                    {"path": "/updated_at", "order": "descending"}
                ]
            ],
            "spatialIndexes": [],
...
class ContainerManager:
    ...
    async def create_branch_aware_container(
        ...
        try:
            ...
            if not throughput_config:
                throughput_config = ContainerConfiguration.get_throughput_configuration()
    ...
    async def update_container_indexing(
        self,
        container_name: str,
        new_indexing_policy: Optional[Dict[str, Any]] = None
    ) -> bool:
        """
    ...
    async def get_container_metrics(self, container_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            metrics = {
                ...
                "properties": {
                    "partition_key_path": properties.get("partitionKey", {}).get("paths", []),
                    "default_ttl": properties.get("defaultTtl"),
                    "indexing_mode": properties.get("indexingPolicy", {}).get("indexingMode")
                },
...
class PerformanceMonitor:
    ...
    async def start_monitoring(
        ...
        while self._monitoring_active:
            try:
                ...
                for container_name in container_names:
                    metrics = await self.container_manager.get_container_metrics(container_name)
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        if partition_dist:
            ...
            avg_docs_per_partition = total_docs / len(partition_dist) if partition_dist else 0
            
            hot_partitions = [
                p["partition_key"] for p in partition_dist
        ...
        report = {
            "summary": {
                "total_containers_monitored": len(set(m["container_name"] for m in self._metrics_history)),
                "metrics_collected": len(self._metrics_history),
                "monitoring_duration_hours": len(self._metrics_history) * 5 / 60,  # Assuming 5-min intervals
            },
            ...
            "partition_analysis": {
                ...
                "partition_balance_score": 1.0 - (len(hot_partitions) + len(cold_partitions)) / max(len(partition_dist), 1)
            },
            "recommendations": self._generate_recommendations(latest_metrics, hot_partitions, cold_partitions),
            "timestamp": datetime.now(timezone.utc).isoformat()
        }
    ...
    def _generate_recommendations(
        ...
        if not recommendations:
            recommendations.append("Container performance is well-balanced. Continue monitoring.")
