...
class MosaicSettings(BaseSettings):
    ...
    azure_cosmos_container_name: str = Field(
        default="knowledge", 
        description="Cosmos DB container name for vector search"
    )
    azure_cosmos_memory_container: str = Field(
        default="memory", 
        description="Cosmos DB container name for memory storage"
    )
    ...
    azure_redis_ssl: bool = Field(default=True, description="Use SSL for Redis connection")
    ...
    semantic_kernel_log_level: str = Field(
        default="INFO", 
        description="Semantic Kernel logging level"
    )
    ...
    memory_importance_threshold: float = Field(
        default=0.7, 
        description="Importance threshold for memory retention"
    )
    short_term_memory_ttl: int = Field(
        default=86400, 
        description="Short-term memory TTL in seconds (24 hours)"
    )
    ...
    rerank_top_k: int = Field(
        default=10, 
        description="Top K results to return after reranking"
    )
