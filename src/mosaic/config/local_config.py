...
class LocalDevelopmentConfig(BaseSettings):
    ...
    cosmos_disable_ssl: bool = Field(default=True, env="COSMOS_DB_DISABLE_SSL_VERIFICATION")
    
    # Container names
    libraries_container: str = Field(default="libraries", env="MOSAIC_LIBRARIES_CONTAINER")
    memories_container: str = Field(default="memories", env="MOSAIC_MEMORIES_CONTAINER")
    documents_container: str = Field(default="documents", env="MOSAIC_DOCUMENTS_CONTAINER")
    config_container: str = Field(default="config", env="MOSAIC_CONFIG_CONTAINER")
    
    # Redis Configuration  
    redis_host: str = Field(default="localhost", env="REDIS_HOST")
    redis_port: int = Field(default=6379, env="REDIS_PORT")
    redis_password: str = Field(env="REDIS_PASSWORD", description="Redis password - must be set in environment")
    ...
    openai_api_version: str = Field(default="2024-02-15-preview", env="AZURE_OPENAI_API_VERSION")
    openai_deployment: str = Field(default="gpt-4o", env="AZURE_OPENAI_DEPLOYMENT_NAME")
    openai_embedding_deployment: str = Field(default="text-embedding-3-small", env="AZURE_OPENAI_EMBEDDING_DEPLOYMENT")
    ...
    def validate_required_settings(self) -> list[str]:
        ...
        if not self.openai_endpoint:
            errors.append("AZURE_OPENAI_ENDPOINT is required even for local development")
