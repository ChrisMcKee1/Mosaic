...
class MemoryPlugin:
    ...
    @sk_function(
        description="Save memory to multi-layered storage system",
        name="save"
    )
    ...
    async def save(self, session_id: str, content: str, memory_type: str) -> Dict[str, Any]:
        """
    ...
    async def retrieve(
        ...
        try:
            ...
            sorted_memories = sorted(
                all_memories,
                key=lambda m: (m.importance_score, self._calculate_text_similarity(query, m.content)),
                reverse=True
            )
    ...
    async def _calculate_importance(self, content: str, memory_type: str) -> float:
        ...
        important_keywords = [
            ...
            "decision", "agreed", "confirmed", "final", "conclusion"
        ]
        
        keyword_bonus = sum(0.1 for keyword in important_keywords if keyword in content_lower)
    ...
    async def _save_to_redis(self, memory_entry: MemoryEntry) -> None:
        ...
        await self.redis_client.setex(
            key, 
            self.settings.short_term_memory_ttl, 
            value
        )
    ...
    async def _save_to_cosmos(self, memory_entry: MemoryEntry) -> None:
        ...
        await asyncio.to_thread(
            self.memory_container.create_item,
            body=item
        )
    ...
    async def _search_cosmos(
        ...
        try:
            ...
            items = list(await asyncio.to_thread(
                lambda: list(self.memory_container.query_items(
                    query=query["query"],
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        try:
            if self.memory_container:
                # Simple test query
                list(self.memory_container.query_items(
