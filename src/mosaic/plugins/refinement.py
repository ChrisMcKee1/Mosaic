...
class RefinementPlugin:
    ...
    async def _validate_ml_endpoint(self) -> None:
        ...
        try:
            ...
            logger.warning(
                f"ML endpoint validation failed: {e}. Using fallback scoring."
            )
    ...
    @sk_function_context_parameter(
        name="query", description="Original search query", type_="str"
    )
    @sk_function_context_parameter(
        name="documents",
    ...
    async def _rerank_with_crossencoder(
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    f"ML endpoint error {response.status_code}: {response.text}"
                )
            ...
            reranked = sorted(
                documents, key=lambda doc: doc_scores.get(doc.id, 0.0), reverse=True
            )
    ...
    async def _rerank_with_fallback(
        ...
        try:
            ...
            for doc in documents:
                ...
                length_penalty = max(
                    0, 1 - (len(doc.content) / 2000)
                )  # Penalty after 2000 chars
