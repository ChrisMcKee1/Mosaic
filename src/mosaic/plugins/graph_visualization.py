...
class GraphVisualizationPlugin:
    ...
    async def visualize_repository_structure(
        ...
        try:
            ...
            relationships_df = await self.data_service.get_repository_relationships(
                repository_url
            )
            ...
            logger.info(
                f"Successfully created visualization with {len(entities_df)} nodes and {len(relationships_df)} relationships"
            )
            return html_output
    ...
    async def visualize_code_dependencies(
        ...
        try:
            ...
            html_output = VG.render()
            logger.info(
                f"Created dependency visualization with {len(entities_df)} nodes"
            )
            return html_output
    ...
    async def visualize_knowledge_graph(
        ...
        try:
            ...
            (
                entities_df,
                relationships_df,
            ) = await self.data_service.get_semantic_knowledge_graph(
                repository_url,
                max_nodes=max_nodes,
                similarity_threshold=0.7 if include_semantic_similarity else 1.0,
            )

            if entities_df.empty:
                return self._create_empty_graph_message(
                    repository_url, "knowledge graph"
                )
            ...
            if "importance_score" in entities_df.columns:
                VG.resize_nodes(
                    property="importance_score", node_radius_min_max=(6, 28)
                )
    ...
    async def create_custom_graph(
        ...
        try:
            ...
            if (
                styling.get("color_property")
                and styling["color_property"] in entities_df.columns
            ):
                VG.color_nodes(property=styling["color_property"])

            if (
                styling.get("size_property")
                and styling["size_property"] in entities_df.columns
            ):
                min_size = styling.get("min_node_size", 5)
                max_size = styling.get("max_node_size", 25)
                VG.resize_nodes(
                    property=styling["size_property"],
                    node_radius_min_max=(min_size, max_size),
                )
        ...
        try:
            ...
            if total_entities == 0:
                recommendations.append(
                    "Repository not ingested yet - run ingestion service first"
                )
            elif total_entities < 50:
                recommendations.append(
                    "Small repository - all visualization modes should work well"
                )
            elif total_entities > 500:
                recommendations.append(
                    "Large repository - consider using filters or limiting node count"
                )

            if total_relationships == 0:
                recommendations.append(
                    "No relationships found - dependency visualization may be limited"
                )
    ...
    def _create_empty_graph_message(
        self, repository_url: str, graph_type: str = "repository"
    ) -> str:
