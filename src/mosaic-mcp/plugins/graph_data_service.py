...
class GraphDataService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.containers = {
                ...
                "relationships": self.database.get_container_client(
                    "code_relationships"
                ),
    ...
    async def get_repository_entities(
        ...
        try:
            ...
            items = list(
                self.containers["entities"].query_items(
                    query=query,
            ...
            if (
                "complexity_score" in df.columns
                and not df["complexity_score"].isna().all()
            ):
            ...
            logger.info(
                f"Retrieved {len(df)} entities for repository: {repository_url}"
            )
            return df
    ...
    async def get_repository_relationships(
        ...
        try:
            ...
            items = list(
                self.containers["relationships"].query_items(
                    query=query,
                    parameters=parameters,
                    enable_cross_partition_query=True,
                )
            ...
            if not items:
                logger.warning(
                    f"No relationships found for repository: {repository_url}"
                )
                return pd.DataFrame()
            ...
            logger.info(
                f"Retrieved {len(df)} relationships for repository: {repository_url}"
            )
            return df
    ...
    async def get_dependency_data(
        self, repository_url: str, include_external: bool = True
        ...
        try:
            ...
            if not relationships_df.empty:
                relationships_df["source"] = pd.to_numeric(
                    relationships_df["source"], errors="coerce"
                )
                relationships_df["target"] = pd.to_numeric(
                    relationships_df["target"], errors="coerce"
                )
            ...
            logger.info(
                f"Retrieved dependency data: {len(entities_df)} entities, {len(relationships_df)} relationships"
            )
        ...
        try:
            ...
            if not entities_df.empty:
                ...
                if "importance_score" in entities_df.columns:
                    entities_df["size"] = (
                        entities_df["importance_score"] * 20 + 8
            ...
            logger.info(
                f"Retrieved semantic knowledge graph: {len(entities_df)} entities, {len(relationships_df)} relationships"
            )
            return entities_df, relationships_df
    ...
    async def get_repository_stats(self, repository_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "entity_stats": {
                    item["entity_type"]: item["count"] for item in entity_stats
                },
                "relationship_stats": {
                    item["relationship_type"]: item["count"] for item in rel_stats
                },
