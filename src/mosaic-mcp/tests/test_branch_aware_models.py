...
class TestFileVersionModel:
    ...
    def test_file_version_with_ai_analysis(self):
        ...
        version_id = generate_file_version_id(
            file_identity_id, "feature", "def456abc123"
        )
...
class TestBranchAwarePartitionConfig:
    ...
    def test_config_creation_defaults(self):
        ...
        config = BranchAwarePartitionConfig(strategy=PartitionStrategy.BRANCH_NAME)
...
class TestModelRelationships:
    ...
    def test_complete_entity_hierarchy(self):
        ...
        file_version_id = generate_file_version_id(
            file_identity_id, "feature", "abc123"
        )
        ...
        assert (
            branch.branch_name == file_version.branch_name == entity_version.branch_name
        )
        assert (
            file_identity.id
            == file_version.file_identity_id
            == entity_version.file_identity_id
        )
        assert file_version.id == entity_version.file_version_id
        assert (
            branch.partition_key
            == file_version.partition_key
            == entity_version.partition_key
        )
    ...
    def test_partition_key_consistency(self):
        ...
        assert (
            branch.partition_key == file_version.partition_key == entity.partition_key
        )
...
if __name__ == "__main__":
    ...
    pytest.main(
        [
            __file__,
            "-v",
            "--cov=mosaic_mcp.models.branch_aware_models",
