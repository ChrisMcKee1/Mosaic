...
class MockMosaicMCPServer:
    ...
    def _setup_tools(self):
        ...
        @self.app.tool()
        async def query_code_graph(library_id: str, relationship_type: str) -> List[Dict[str, Any]]:
            """Mock code graph query tool."""
            return [
                {"id": library_id, "type": relationship_type, "related": ["lib1", "lib2"]}
            ]
...
class TestMCPPerformance:
    ...
    async def test_concurrent_request_handling(self, mcp_server):
        ...
        tasks = [
            mcp_server.app.tools["hybrid_search"](f"query {i}")
            for i in range(10)
        ]
