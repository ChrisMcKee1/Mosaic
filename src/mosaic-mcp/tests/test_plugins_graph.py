...
class MockGraphPlugin:
    ...
    async def natural_language_query(
        ...
        return {
            ...
            "visualization": "<div>Mock visualization</div>"
    ...
    async def execute_sparql_query(
        ...
        return {
            ...
            "visualization": "<div>SPARQL visualization</div>"
            if include_visualization
            else None,
        }
    ...
    async def discover_graph_schema(
        ...
        if include_counts:
            schema_data["counts"] = {"Class": 300, "Property": 200, "Individual": 500}
...
class TestNaturalLanguageQuery:
    ...
    async def test_natural_language_query_with_max_results(self, graph_plugin):
        ...
        result = await graph_plugin.natural_language_query(query, max_results=5)
...
class TestSPARQLQuery:
    ...
    async def test_sparql_query_result_limiting(self, graph_plugin):
        ...
        result = await graph_plugin.execute_sparql_query(sparql_query, max_results=1)
...
class TestGraphVisualization:
    ...
    async def test_visualize_graph_results_different_layouts(self, graph_plugin):
        ...
        for layout in ["force", "circular", "hierarchical"]:
            html = await graph_plugin.visualize_graph_results(results, layout=layout)
...
class TestGraphPluginEdgeCases:
    ...
    async def test_concurrent_queries(self, graph_plugin):
        ...
        queries = ["Find all classes", "Show properties", "List individuals"]

        # Execute queries concurrently
        tasks = [graph_plugin.natural_language_query(query) for query in queries]
