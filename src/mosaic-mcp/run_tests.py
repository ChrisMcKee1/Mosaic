...
def check_dependencies() -> bool:
    ...
    if missing_packages:
        ...
        print(
            "💡 Install them with: pip install pytest pytest-asyncio pytest-cov pytest-mock"
        )
        return False
...
def print_test_summary(results: List[Dict], test_stats: Dict, coverage_data: Dict):
    ...
    for result in results:
        ...
        print(
            f"   • {result['type'].capitalize()} Tests: {status} ({result['duration']:.1f}s)"
        )
    ...
    if coverage_data:
        ...
        print(
            f"   • Coverage Percentage: {coverage_data.get('percent_covered', 0):.1f}%"
        )
