...
def sample_chat_history():
    ...
    return [
        ...
        ("assistant", "The system includes 5 specialized AI agents: GitSleuth, CodeParser, GraphArchitect, DocuWriter, and GraphAuditor."),
        ("user", "Show me the ingestion status"),
        ("assistant", "Ingestion system status: 38 files processed, 696 entities extracted, system ready.")
    ]
    ...
    return {
        ...
        "vector_rag": {
            ...
            "results": {
                ...
                "semantic_matches": 5,
                "top_matches": [
                    {"name": "similar_component", "similarity": 0.85}
                ]
            }
        },
        "graph_rag": {
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "ui: marks tests as UI component tests"
    )
    config.addinivalue_line(
    ...
    config.addinivalue_line(
        "markers", "omnirag: marks tests as OmniRAG plugin tests"
    )
...
EXPECTED_CATEGORIES = ["server", "plugin", "ingestion", "ai_agent", "config", "model", "ui"]
