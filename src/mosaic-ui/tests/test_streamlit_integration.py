...
class MockStreamlitApp:
    ...
    def markdown(self, text, unsafe_allow_html=False):
        """Mock st.markdown()."""
        self.components.append(("markdown", text, {"unsafe_allow_html": unsafe_allow_html}))
        return self
    ...
    def columns(self, spec):
        ...
        for i, col in enumerate(cols):
            ...
            col.selectbox = lambda label, options, **kwargs: self.selectbox(label, options, **kwargs)
    ...
    def sidebar(self):
        """Mock st.sidebar."""
        sidebar = MagicMock()
        sidebar.markdown = lambda text, **kwargs: self.sidebar_components.append(("markdown", text))
        sidebar.button = lambda label, **kwargs: self.sidebar_components.append(("button", label))
        sidebar.selectbox = lambda label, options, **kwargs: self.sidebar_components.append(("selectbox", label, options))
        return sidebar
...
class TestStreamlitSessionState:
    ...
    def test_session_state_updates(self, mock_streamlit):
        ...
        # Update entities
        test_entities = [
            {"id": "test1", "name": "Test Entity", "category": "test"}
        ]
...
class TestStreamlitComponents:
    ...
    def test_header_components(self, mock_streamlit):
        ...
        subheaders = [comp for comp in mock_streamlit.components if comp[0] == "subheader"]
    ...
    def test_markdown_rendering(self, mock_streamlit):
        ...
        markdown_components = [comp for comp in mock_streamlit.components if comp[0] == "markdown"]
    ...
    def test_form_components(self, mock_streamlit):
        ...
        # Verify components
        selectboxes = [comp for comp in mock_streamlit.components if comp[0] == "selectbox"]
        text_inputs = [comp for comp in mock_streamlit.components if comp[0] == "text_input"]
...
class TestMessageHandling:
    ...
    def test_success_messages(self, mock_streamlit):
        ...
        success_messages = [msg for msg in mock_streamlit.messages if msg[0] == "success"]
    ...
    def test_warning_messages(self, mock_streamlit):
        ...
        warning_messages = [msg for msg in mock_streamlit.messages if msg[0] == "warning"]
    ...
    def test_multiple_messages(self, mock_streamlit):
        ...
        success_count = len([msg for msg in mock_streamlit.messages if msg[0] == "success"])
        error_count = len([msg for msg in mock_streamlit.messages if msg[0] == "error"])
        warning_count = len([msg for msg in mock_streamlit.messages if msg[0] == "warning"])
...
class TestChatInterface:
    ...
    def test_chat_history_display(self, mock_streamlit, sample_chat_history):
        ...
        chat_components = [comp for comp in mock_streamlit.components if comp[0] == "markdown"]
    ...
    def test_chat_input_handling(self, mock_streamlit):
        ...
        user_input = mock_streamlit.text_input(
            "Ask a question:",
            value="What AI agents are available?"
        )
    ...
    def test_chat_message_addition(self, mock_streamlit):
        ...
        mock_streamlit.session_state.chat_history.append(("assistant", assistant_response))
        
        assert len(mock_streamlit.session_state.chat_history) == 2
        assert mock_streamlit.session_state.chat_history[0] == ("user", user_message)
        assert mock_streamlit.session_state.chat_history[1] == ("assistant", assistant_response)
...
class TestErrorHandlingUI:
    ...
    def test_query_error_handling(self, mock_streamlit):
        ...
        assert len(mock_streamlit.session_state.chat_history) == 2
        assert "Error processing query" in mock_streamlit.session_state.chat_history[1][1]
...
class TestNavigationAndState:
    ...
    def test_dynamic_content_updates(self, mock_streamlit):
        ...
        mock_streamlit.session_state.selected_node = {"id": "new_node", "name": "New Node"}
...
class TestPerformanceAndResponsiveness:
    ...
    def test_large_dataset_rendering(self, mock_streamlit, performance_test_data):
        ...
        mock_streamlit.session_state.relationships = performance_test_data["relationships"]
