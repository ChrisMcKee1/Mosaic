...
class TestRenderingPerformance:
    ...
    def test_entity_list_rendering_performance(self, performance_monitor, large_graph_dataset):
        ...
        for entity in entities:
            ...
            rendered_entity = {
                ...
                "category_color": "#4ecdc4" if entity["category"] == "plugin" else "#ff6b6b",
        ...
        assert render_time < 0.5, f"Entity rendering took {render_time:.3f}s, should be < 0.5s"
        ...
        assert estimated_memory_mb < 50, f"Memory usage {estimated_memory_mb:.1f}MB too high"
    
    def test_graph_visualization_performance(self, performance_monitor, large_graph_dataset):
        ...
        # Performance assertions
        assert render_time < 0.3, f"Graph rendering took {render_time:.3f}s, should be < 0.3s"
    ...
    def test_component_loading_performance(self, performance_monitor):
        ...
        components_to_test = [
            "header", "sidebar", "main_graph", "chat_interface", 
            "metrics_panel", "status_indicators", "quick_actions"
        ]
        ...
        # Performance assertions
        assert total_time < 0.1, f"Total component loading took {total_time:.3f}s, should be < 0.1s"
        
        # Individual component checks
        for component, duration in component_times.items():
            assert duration < 0.01, f"Component {component} took {duration:.3f}s, should be < 0.01s"
    ...
    def test_concurrent_rendering_performance(self, performance_monitor):
        ...
        assert max_render_time < 0.05, f"Max concurrent render time {max_render_time:.3f}s too high"
        assert avg_render_time < 0.02, f"Average concurrent render time {avg_render_time:.3f}s too high"


class TestMemoryUsageOptimization:
    """Test memory usage optimization."""
    
    def test_entity_data_memory_efficiency(self, performance_monitor, large_graph_dataset):
        ...
        # Assertions
        assert memory_savings > 20, f"Memory savings only {memory_savings:.1f}%, should be > 20%"
    ...
    def test_chat_history_memory_management(self, performance_monitor, extensive_chat_history):
        ...
        for i, (role, message) in enumerate(truncated_history):
            if i < len(truncated_history) - 20:  # Compress older messages
                compressed_message = message[:100] + "..." if len(message) > 100 else message
        ...
        assert truncation_savings > 50, f"Truncation savings {truncation_savings:.1f}% insufficient"
        assert compression_savings > 10, f"Compression savings {compression_savings:.1f}% insufficient"
        assert compressed_mb < 5, f"Final memory usage {compressed_mb:.1f}MB too high"
    
    def test_graph_data_memory_optimization(self, performance_monitor, large_graph_dataset):
        ...
        # Performance assertions
        assert memory_reduction > 60, f"Memory reduction {memory_reduction:.1f}% insufficient"
...
class TestResponseTimeOptimization:
    ...
    async def test_query_response_performance(self, performance_monitor):
        ...
        async def mock_vector_search(query):
            ...
            return [
                {"id": f"vector_{i}", "similarity": 0.9 - (i * 0.1)}
                for i in range(5)
            ]
    ...
    def test_graph_update_performance(self, performance_monitor):
        ...
        current_edges = [{"source": f"node_{i}", "target": f"node_{i+1}"} for i in range(49)]
        ...
        update_scenarios = [
            ...
            ("update_nodes", [{"id": "node_2", "name": "Updated Node 2", "color": "red"}]),
            # Add edges
            ...
            if operation == "add_nodes":
                ...
                for edge_to_remove in data:
                    current_edges = [e for e in current_edges 
                                   if not (e["source"] == edge_to_remove["source"] and 
                                          e["target"] == edge_to_remove["target"])]
        ...
        assert max_update_time < 0.01, f"Max update time {max_update_time:.3f}s too high"
        assert avg_update_time < 0.005, f"Average update time {avg_update_time:.3f}s too high"
    ...
    def test_ui_interaction_responsiveness(self, performance_monitor):
        ...
        def simulate_user_interaction(interaction_type):
            ...
            if interaction_type == "node_click":
                ...
                selected_node = {"id": "test_node", "name": "Test Node", "details": "..." * 100}
        ...
        for interaction_type, metrics in interaction_times.items():
            assert metrics["max"] < 0.02, f"{interaction_type} max time {metrics['max']:.3f}s too high"
            assert metrics["avg"] < 0.01, f"{interaction_type} avg time {metrics['avg']:.3f}s too high"
...
class TestScalabilityLimits:
    ...
    def test_maximum_entity_count(self, performance_monitor):
        ...
        for i, (count, time_taken) in enumerate(performance_degradation):
            if i > 0:
                ...
                assert time_ratio <= count_ratio * 1.5, f"Performance degradation too high at {count} entities"
        
        # Maximum limits
        final_count, final_time = performance_degradation[-1]
        assert final_time < 1.0, f"Processing {final_count} entities took {final_time:.2f}s, too slow"
    ...
    def test_chat_history_scalability(self, performance_monitor):
        ...
        assert max_processing_time < 0.1, f"Chat processing time {max_processing_time:.3f}s too high"
        ...
        if len(processing_times) > 1:
            ...
            assert degradation_ratio < 5, f"Performance degradation ratio {degradation_ratio:.1f}x too high"
    ...
    def test_concurrent_user_simulation(self, performance_monitor):
        ...
        for user_count in user_counts:
            ...
            # Performance assertions
            assert total_time < 2.0, f"Concurrent sessions with {user_count} users took {total_time:.2f}s"
            assert len(session_results) == user_count, f"Not all user sessions completed"
...
class TestResourceUtilization:
    ...
    def test_memory_leak_detection(self, performance_monitor):
        ...
        for cycle in range(10):
            ...
            cycle_end_memory = cycle_start_memory + (cycle * 0.1)  # Simulate small growth
        ...
        assert growth_per_cycle < 1.0, f"Memory growth {growth_per_cycle:.2f}MB/cycle indicates leak"
    ...
    def test_cpu_utilization_efficiency(self, performance_monitor):
        ...
        for op_name, execution_time in operation_times.items():
            assert execution_time < 0.1, f"Operation {op_name} took {execution_time:.3f}s, too slow"
    ...
    def test_network_request_efficiency(self, performance_monitor):
        ...
        async def test_request_patterns():
            ...
            tasks = [
                mock_api_request(f"/api/data/{i}", data_size_kb=10)
                for i in range(5)
            ]
        ...
        efficiency_improvement = (sequential_time - concurrent_time) / sequential_time * 100
        assert efficiency_improvement > 50, f"Concurrency improvement only {efficiency_improvement:.1f}%"
        
        assert concurrent_time < 0.5, f"Concurrent requests took {concurrent_time:.3f}s, too slow"
