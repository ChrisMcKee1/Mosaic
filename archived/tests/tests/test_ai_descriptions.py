...
class TestAIPoweredEntityDescriptions:
    ...
    async def test_caching_mechanism(self, ingestion_plugin):
        ...
        with patch.object(
            ingestion_plugin,
            "_call_semantic_kernel_for_description",
    ...
    async def test_retry_logic_with_failures(self, ingestion_plugin):
        ...
        with patch.object(
            ...
            "_call_semantic_kernel_for_description",
            new_callable=AsyncMock,
            side_effect=mock_call_with_failures,
        ):
    ...
    async def test_batch_processing(self, ingestion_plugin, sample_entities):
        ...
        with patch.object(
            ...
            enhanced_lookup = await ingestion_plugin._process_entities_in_batches(
                candidates
            )
            ...
            # Verify descriptions
            assert (
                enhanced_lookup["test_func_1"]["ai_description"]
                == descriptions["calculate_fibonacci"]
            )
            assert (
                enhanced_lookup["test_class_1"]["ai_description"]
                == descriptions["DatabaseManager"]
            )
    ...
    async def test_specialized_prompts(self, ingestion_plugin):
        ...
        for entity_type, expected_start in test_cases:
            ...
            assert prompt.startswith(expected_start), (
                f"Unexpected prompt for {entity_type}: {prompt}"
            )
    ...
    async def test_full_enhancement_pipeline(self, ingestion_plugin, sample_entities):
        ...
        async def mock_kernel_invoke(*args, **kwargs):
            ...
            return mock_descriptions.get(
                entity_name, f"AI description for {entity_name}"
            )
        ...
        enhanced_entities = (
            await ingestion_plugin._enhance_entities_with_ai_descriptions(
                sample_entities
            )
        )
        ...
        fibonacci_entity = next(
            e for e in enhanced_entities if e.get("name") == "calculate_fibonacci"
        )
    ...
    async def test_error_handling_graceful_degradation(
        self, ingestion_plugin, sample_entities
    ):
