...
class TestRealRepositoryCloning:
    ...
    async def test_clone_mosaic_repository_main_branch(self, ingestion_plugin):
        ...
        try:
            ...
            for expected_dir in expected_dirs:
                ...
                assert os.path.isdir(dir_path), f"Expected {expected_dir} to be a directory"
            
            # Verify the ingestion plugin file exists and has our enhanced method
            ingestion_file = os.path.join(repo_path, "src", "ingestion_service", "plugins", "ingestion.py")
            ...
            print(f"📁 Repository contains {len(os.listdir(repo_path))} top-level items")
    ...
    async def test_clone_with_different_branch_fallback(self, ingestion_plugin):
        ...
        assert any(keyword in error_msg.lower() for keyword in [
            "branch", "not found", "does not exist", "repository"
        ])
    ...
    async def test_repository_analysis_preview(self, ingestion_plugin):
        ...
        try:
            ...
            print(f"✅ Found {len(found_enhancements)}/{len(enhancements)} expected enhancements")
            
            # Should find most of our enhancements
            assert len(found_enhancements) >= 6, f"Expected most enhancements, found: {found_enhancements}"
