...
class TestKnowledgeBasePopulation:
    ...
    @pytest.mark.asyncio
    async def test_populate_knowledge_base_success(self, plugin, sample_entities, sample_relationships):
        ...
        assert plugin.knowledge_container.upsert_item.call_count == 3  # 2 entities + 1 relationship
        
        # Verify entity documents structure
        entity_calls = [call for call in plugin.knowledge_container.upsert_item.call_args_list
                       if call[0][0].get("type") == "code_entity"]
        assert len(entity_calls) == 2
        
        # Verify relationship documents structure
        rel_calls = [call for call in plugin.knowledge_container.upsert_item.call_args_list
                    if call[0][0].get("type") == "relationship"]
        assert len(rel_calls) == 1

    @pytest.mark.asyncio
    async def test_generate_and_store_entity_embeddings_batch_processing(self, plugin, sample_entities):
    ...
    @pytest.mark.asyncio
    async def test_embedding_generation_with_ai_descriptions(self, plugin, sample_entities):
        ...
        assert "UserService class: Service class for managing user operations" in call_args[1]
    ...
    async def test_error_handling_embedding_failure(self, plugin, sample_entities):
        ...
        plugin.embedding_service.generate_embeddings.side_effect = Exception("Embedding service error")
    ...
    async def test_error_handling_cosmos_db_failure(self, plugin, sample_entities):
        ...
        plugin.knowledge_container.upsert_item.side_effect = Exception("Cosmos DB error")
    ...
    @pytest.mark.asyncio
    async def test_error_handling_relationship_storage_failure(self, plugin, sample_relationships):
        """Test error handling when relationship storage fails."""
        plugin.knowledge_container = AsyncMock()
        
        # Mock Cosmos DB failure for relationships
        plugin.knowledge_container.upsert_item.side_effect = Exception("Relationship storage error")
    ...
    async def test_concurrent_batch_processing(self, plugin):
        ...
        entities1 = [{"id": f"batch1_{i}", "entity_type": "function", "name": f"func1_{i}", 
                     "language": "python", "file_path": f"/app/batch1_{i}.py", 
                     "content": f"def func1_{i}(): pass", "timestamp": datetime.now().isoformat()}
                    for i in range(50)]
        
        entities2 = [{"id": f"batch2_{i}", "entity_type": "class", "name": f"class2_{i}", 
                     "language": "python", "file_path": f"/app/batch2_{i}.py", 
                     "content": f"class Class2_{i}: pass", "timestamp": datetime.now().isoformat()}
                    for i in range(75)]
