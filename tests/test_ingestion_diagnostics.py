...
async def test_ingestion_service_creation():
    ...
    try:
        ...
        try:
            ...
            print(f"   ✅ Tree-sitter languages initialized: {len(languages)} languages")
        except Exception as e:
            print(f"   ⚠️  Tree-sitter initialization failed: {e}")
            print("      Some file parsing may not work, but basic functionality should still work")
...
async def main():
    ...
    if all_passed:
        ...
        print("   python3 -m ingestion_service.main --repository-url https://github.com/ChrisMcKee1/Mosaic --branch main")
    else:
        print("⚠️  Some tests failed. Please fix the issues above before running the full service.")
        print("\n🔧 Common fixes:")
        print("   1. Install missing dependencies: pip install -r requirements.txt")
        print("   2. Copy .env.local to .env and add Azure OpenAI credentials")
        print("   3. Ensure Docker containers are running: ./scripts/start-local-dev.sh")
