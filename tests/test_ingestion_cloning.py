...
class TestRepositoryCloning:
    ...
    @pytest.mark.asyncio
    async def test_successful_public_repository_clone(
        self, mock_ingestion_plugin, clean_env
    ):
        ...
        with (
            ...
            result = await mock_ingestion_plugin._clone_repository(
                "https://github.com/user/public-repo.git", "main"
            )
    ...
    async def test_github_token_authentication(self, mock_ingestion_plugin, clean_env):
        ...
        with (
            ...
            assert (
                ...
                in call_args[1]["env"]["GIT_HTTP_EXTRAHEADER"]
            )
    ...
    async def test_username_password_authentication(
        ...
        with (
            ...
            await mock_ingestion_plugin._clone_repository(
                "https://gitlab.com/user/private-repo.git", "main"
            )
    ...
    @pytest.mark.asyncio
    async def test_legacy_git_token_authentication(
        self, mock_ingestion_plugin, clean_env
    ):
        ...
        with (
            ...
            call_args = mock_clone.call_args
            assert (
                "Authorization: Basic legacy_token_base64"
                in call_args[1]["env"]["GIT_HTTP_EXTRAHEADER"]
            )

    @pytest.mark.asyncio
    async def test_authentication_failure_error_handling(
        self, mock_ingestion_plugin, clean_env
    ):
    ...
    @pytest.mark.asyncio
    async def test_repository_not_found_error_handling(
        self, mock_ingestion_plugin, clean_env
    ):
        ...
        with (
            ...
            with pytest.raises(GitCommandError) as exc_info:
                await mock_ingestion_plugin._clone_repository(
                    "https://github.com/user/nonexistent-repo.git", "main"
                )
    ...
    async def test_network_timeout_error_handling(
        ...
        with (
            ...
            with pytest.raises(TimeoutError) as exc_info:
                await mock_ingestion_plugin._clone_repository(
                    "https://github.com/user/large-repo.git", "main"
                )
    ...
    @pytest.mark.asyncio
    async def test_invalid_repository_error_handling(
        self, mock_ingestion_plugin, clean_env
    ):
        ...
        with (
            ...
            with pytest.raises(InvalidGitRepositoryError) as exc_info:
                await mock_ingestion_plugin._clone_repository(
                    "https://github.com/user/empty-repo.git", "main"
                )
    ...
    @pytest.mark.asyncio
    async def test_secure_temporary_directory_permissions(
        self, mock_ingestion_plugin, clean_env
    ):
